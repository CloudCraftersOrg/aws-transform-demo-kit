...
| `generate.py` | Emits `out/transform-demo-assessment.zip` (+ the loose CSVs for inspection). `--check` validates without writing. |
...
# -> inventory/out/transform-demo-assessment.zip  (mpa_servers.csv + network_connections.csv + ASSESSMENT_INTENT.md)
```
...
1. AWS Transform (us-east-1) → workspace → chat "assess workloads for migration
   readiness".
2. Upload **`transform-demo-assessment.zip`**. The connections file is only processed
